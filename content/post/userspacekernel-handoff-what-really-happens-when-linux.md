...
To choose the correct database, modprobe needs the right kernel:
...
```text
kernel/drivers/hid/hid-logitech.ko: usbhid.ko hid.ko usbcore.ko usb-common.ko
```

The modules.dep file contains entries like this:

Then it resolves dependencies recursively and builds a load order:

```text
hid-logitech.ko → usbhid.ko → hid.ko → usbcore.ko → usb-common.ko
```
...
```bash
modprobe hid-logitech debug=2
```

Or in configuration files:

```text
options hid-logitech debug=2
```
